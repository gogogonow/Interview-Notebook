...
前序遍历的第一个值为根节点的值，使用这个值将中序遍历结果分成两部分，左部分为树的左子树中序遍历结果，右部分为树的右子树中序遍历的结果。（可以根据先序遍历和中序遍历求解树；也可以根据中序和后序求解二叉树；但是不能根据先序和后序构建二叉树，因为中序可以切割得到左右子树。先序、中序和后序、中序求解区别在于从前和从后取根。）
